...
class Solution(object):
    ...
    def rewrite(self, org, seqs):
        ...
        """
        # idea:
        # seqs pairs inside org pair
        idx_map = {val: idx for idx, val in enumerate([None] + org)}

        # create sequenced before set
        o_set = set([(m[0], m[1])for m in zip([None] + org, org)])

        for seq in seqs:
            for s in zip([None] + seq, seq):
                if idx_map[s[0]] >= idx_map[s[1]]:
                    return False

                o_set.discard((s[0], s[1]))
        return not o_set
