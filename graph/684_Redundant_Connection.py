...
class Solution(object):
    ...
    def rewrite(self, edges):
        """
        :type edges: List[List[int]]
        :rtype: List[int]
        """
        # uf solution
        # 用dict比較看的清楚 沒有白痴的side effect
        # 不要用 defaultdict, defaultdict在沒有key時會自己加key with zero value.
        graph = dict()
    ...
    def rewrite2(self, edges):
        ...
        keys = "".join(map(chr, (i for i in range(1024))))
        ...
        for e in edges:
            ...
            keys = keys.replace(keys[x], keys[y])
