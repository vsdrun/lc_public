...
class Solution(object):
    ...
    def rewrite(self, num1, num2):
        ...
        for idx, i in enumerate(num1[::-1]):
            mvidx = ~idx
            carry = 0

            for j in num2[::-1]:
                tv = int(i) * int(j) + carry + int(result[mvidx])
                result[mvidx] = str(tv % 10)
                carry = tv / 10
                mvidx -= 1

            if carry:
                result[mvidx] = str(int(result[mvidx]) + carry)

        result[mvidx] = str(carry)
