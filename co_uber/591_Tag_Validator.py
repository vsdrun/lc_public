...
you need to implement a tag validator to parse the code and return whether it
is valid.
...
and has length in range [1,9]. Otherwise, the TAG_NAME is invalid.

A valid TAG_CONTENT may contain other valid closed tags,
...
so even it has some characters that can be parsed as tag (no matter valid
...
(including the any characters mentioned above) only contain letters,
digits, '<','>','/','!','[',']' and ' '.
"""
